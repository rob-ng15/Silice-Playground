...
algorithm main(
    ...
    // MEMMAP_IO <-> MEMORYMANAGER
    uint16  memmapioAddress = uninitialized;
    uint16  memmapioWriteData = uninitialized;
    uint16  memmapioReadData = uninitialized;
    uint1   memmapioWrite = uninitialized;
    uint1   memmapioRead = uninitialized;
    ...
    memmap_io IO_Map (
        ...
        // MEMORYMANAGER
        memoryAddress <: memmapioAddress,
        writeData <: memmapioWriteData,
        memoryWrite <: memmapioWrite,
        memoryRead <: memmapioRead,
        readData :> memmapioReadData
    );

    // Setup Memory
    memorymanager riscvMemory (
        // MEMMAP_IO
            memmapioAddress :> memmapioAddress,
            memmapioWriteData :> memmapioWriteData,
            memmapioWrite :> memmapioWrite,
            memmapioRead :> memmapioRead,
            memmapioReadData <: memmapioReadData
    );

    // RISC-V MEMORY TOGGLES
    riscvMemory.readType := 0;
    riscvMemory.writeType := 0;
    ...
    while(1) {
        ...
        riscvMemory.address = pc;
        riscvMemory.readType = 3;
        ++:
        while( riscvMemory.readComplete == 0 ) { ++: }
        instruction = riscvMemory.readData;
        ++:
        ...
        switch( Utype(instruction).opCode ) {
            ...
            case 7b0000011: {
                ...
                riscvMemory.address = immediateValue + registers[ Itype(instruction).sourceReg1 ];
                riscvMemory.readType = ( Itype(instruction).function3 & 3 ) + 1;
                ++:
                while( riscvMemory.readComplete == 0 ) { ++: }

                    switch( Itype(instruction).function3  ) {
                        case 3b000: { result = { {24{riscvMemory.readData[7,1]}}, riscvMemory.readData[0,8] }; }
                        case 3b001: { result = { {16{riscvMemory.readData[15,1]}}, riscvMemory.readData[0,16] }; }
                        case 3b010: { result = riscvMemory.readData; }
                        case 3b100: { result = { 24b0, riscvMemory.readData[0,8] }; }
                        case 3b101: { result = { 16b0, riscvMemory.readData[0,16] }; }
                    }
            ...
            case 7b0100011: {
                leds = 64;
                // STORE
                riscvMemory.address = { {20{instruction[31,1]}}, Stype(instruction).immediate_bits_11_5, Stype(instruction).immediate_bits_4_0 } + sourceReg1;
                riscvMemory.writeData = sourceReg2;
                riscvMemory.writeType = ( Stype(instruction).function3 & 3 ) + 1;
                ++:
                while( riscvMemory.writeComplete == 0 ) {}
...
algorithm memorymanager(
    ...
    output! uint1   writeComplete,

    output  uint32  readData,
    input   uint2   readType,
    output! uint1   readComplete,

    // MEMORYMANAGER <-> MEMMAP_IO communication
    output! uint16  memmapioAddress,
    output! uint16  memmapioWriteData,
    output! uint1   memmapioWrite,
    output! uint1   memmapioRead,
    input   uint16  memmapioReadData

) <autorun> {

    // memmapio read/write flags
    memmapioWrite := 0;
    memmapioRead := 0;
    ...
    // Signal that read/write has completed
    writeComplete = 1;
    readComplete = 1;
                ...
                if( address[15,1] == 0 ) {
                    ...
                    switch( writeType ) {
                        case 1: {
                            ...
                            ++:
                            switch( address[0,2] ) {
                                case 0: { ram.wdata = { ram.rdata[8,24], writeData[0,8] }; }
                                case 1: { ram.wdata = { ram.rdata[16,16], writeData[0,8], ram.rdata[0,8] }; }
                                case 2: { ram.wdata = { ram.rdata[24,8], writeData[0,8], ram.rdata[0,16] }; }
                                case 3: { ram.wdata = { writeData[0,8], ram.rdata[0,24] }; }
                            }
                        }
                        case 2: {
                            // 16 bit
                            ++:
                            switch( address[1,1] ) {
                                case 0: { ram.wdata = { ram.rdata[16,16], writeData[0,16] }; }
                                case 1: { ram.wdata = { writeData[0,16], ram.rdata[0,16] }; }
                            }
                        }
                        case 3: {
                            // 32 bit
                    ...
                    // I/O
                    memmapioAddress = address[0,16];
                    memmapioWriteData = writeData[0,16];
                    memmapioWrite = 1;
                    ++:
                }
//            } else {
...
//            if( address[16,16] == 0 ) {
                ...
                if( address[15,1] == 0 ) {
                    ...
                    switch( readType ) {
                        case 1: {
                            switch( address[0,2] ) {
                                case 0: { readData = { 24b0, ram.rdata[0,8] }; }
                                case 1: { readData = { 24b0, ram.rdata[8,8] }; }
                                case 2: { readData = { 24b0, ram.rdata[16,8] }; }
                                case 3: { readData = { 24b0, ram.rdata[24,8] }; }
                            }
                        }
                        case 2: {
                            switch( address[1,1] ) {
                                case 0: { readData = { 16b0, ram.rdata[0,16] }; }
                                case 1: { readData = { 16b0, ram.rdata[16,16] }; }
                            }
                        }
                        case 3: {
                            readData = ram.rdata;
