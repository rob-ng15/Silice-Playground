...
void sd_readSector( unsigned int sectorAddress )
{
    tpu_set( 70, 0, 0x40, 0x30 ); tpu_outputnumber_int( sectorAddress );
    ...
    *SDCARD_START = 1;
    ...
    tpu_set( 70, 0, 0x40, 0x0c ); tpu_outputnumber_int( sectorAddress );
}
...
void main()
{
    ...
    copyaddress = (unsigned char *)&partition[0];
    for( i = 0; i < 4*sizeof(PartitionTable); i++ ) {
        *SDCARD_ADDRESS = 0x1be + i;
