...
algorithm memmap_io (
    ...
    pulse1hz p1hz <@clock_IO> (
        counter1hz :> systemClock,
    );
    pulse1hz timer1hz <@clock_IO> ( );

    // 1khz timers (sleepTimer used for sleep command, timer1khz for user purposes)
    pulse1khz sleepTimer <@clock_IO> ( );
    pulse1khz timer1khz <@clock_IO> ( );
    ...
    uart_out uo;
    uart_sender usend <@clock_IO> (
        io      <:> uo,
        uart_tx :>  uart_tx
    );
    uart_in ui;
    uart_receiver urecv <@clock_IO> (
        io      <:> ui,
        uart_rx <:  uart_rx
    );
    ...
    apu apu_processor_L <@clock_IO> (
        staticGenerator <: staticGenerator,
        audio_output :> audio_l
    );
    apu apu_processor_R <@clock_IO> (
        staticGenerator <: staticGenerator,
    ...
    simple_dualport_bram uint8 sdbuffer <@clock_IO,@clock_IO> [512] = uninitialized;
