bitfield gpucommand{
    uint4   gpu_write,
    uint7   gpu_colour,
    uint7   gpu_colour_alt,
    uint4   gpu_dithermode,
    uint11  gpu_x,
    uint11  gpu_y,
    uint11  gpu_param0,
    uint11  gpu_param1,
...
algorithm gpu(
    ...
    // 32 ENTRY GPU COMMAND LIST
    simple_dualport_bram uint88 commandlist[ 32 ] = uninitialised;
    uint5   commandNEXT = 0;
    uint5   commandTOP = 0;
    // COMMAND LIST CONTENTS
    int11   gpu_active_x = uninitialized;
    int11   gpu_active_y = uninitialized;
    int11   gpu_active_param0 = uninitialized;
    int11   gpu_active_param1 = uninitialized;
    int11   gpu_active_param2 = uninitialized;
    int11   gpu_active_param3 = uninitialized;
    ...
    rectangle GPUrectangle(
        x <: gpu_active_x,
        y <: gpu_active_y,
        param0 <: gpu_active_param0,
        param1 <: gpu_active_param1
    );
    line GPUline(
        x <: gpu_active_x,
        y <: gpu_active_y,
    ...
    triangle GPUtriangle(
        ...
        param0 <: gpu_active_param0,
        param1 <: gpu_active_param1,
        param2 <: gpu_active_param2,
        param3 <: gpu_active_param3,
    );
    blit GPUblit(
        x <: gpu_active_x,
        y <: gpu_active_y,
    ...
    // COMMANDLIST READ/WRITE FLAGS
    commandlist.addr0 := commandNEXT;
    commandlist.addr1 := commandTOP;
    commandlist.wenable1 := 1;

    // GPU BUSY - Buffer FULL
    gpu_active := ( ( commandTOP + 1 ) == commandNEXT ) ? 1 : 0;
    ...
    always {
        // GPU COMMAND TO STORE
        if( vector_drawer.gpu_write != 0 ) {
            commandlist.wdata1 = { 4b0010, vector_block_colour, 7b0, 4b0, vector_drawer.gpu_x, vector_drawer.gpu_y, vector_drawer.gpu_param0, vector_drawer.gpu_param1, 11b0, 11b0 };
            commandTOP = commandTOP + 1;
        } else {
            if( gpu_write != 0 ) {
                commandlist.wdata1 = { gpu_write, gpu_colour, gpu_colour_alt, gpu_dithermode, gpu_x, gpu_y, gpu_param0, gpu_param1, gpu_param2, gpu_param3 };
                commandTOP = commandTOP + 1;
            }
        }
    }
    ...
    while(1) {
        if( commandNEXT != commandTOP ) {
            ++:
            // COMMAND LIST CONTENTS
            gpu_active_x = gpucommand(commandlist.rdata0).gpu_x;
            gpu_active_y = gpucommand(commandlist.rdata0).gpu_y;
            gpu_active_param0 = gpucommand(commandlist.rdata0).gpu_param0;
            gpu_active_param1 = gpucommand(commandlist.rdata0).gpu_param1;
            gpu_active_param2 = gpucommand(commandlist.rdata0).gpu_param2;
            ...
            switch( gpucommand(commandlist.rdata0).gpu_write ) {
                case 1: {
                    ...
                    bitmap_x_write = gpucommand(commandlist.rdata0).gpu_x;
                    bitmap_y_write = gpucommand(commandlist.rdata0).gpu_y;
                    bitmap_write = 1;
                }
                ...
                case 3: {
                    ...
                    gpu_active_dithermode = gpucommand(commandlist.rdata0).gpu_dithermode;
