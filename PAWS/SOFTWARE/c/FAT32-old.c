...
unsigned int filebrowser( void ) {
    ...
    while( 1 ) {
        while( !finished) {
            ...
            printw("NEXT DIRECTORY CLUSTER = %d\n",FAT32table[ FAT32directorycluster ]);
            if( FAT32table[ FAT32directorycluster ] >= 0xfffffff8 ) {
                finished = 1;
            } else {
        ...
        while( !new_directory ) {
            ...
            printw(" STARTING CLUSTER @ %d\n",directorynames[present_entry].starting_cluster);
            while( get_buttons() == 1 ) {}
            if( get_buttons() & 64 ) {
                // MOVE RIGHT
...
int main( void ) {
    ...
    for( int i = 0; i < 4; i++ ) {
        printw( "Partition %d, Type = %d, First Sector = %d, Length = %d sectors\n",i,partitions[i].partition_type,partitions[i].start_sector,partitions[i].length_sectors );
    }
    ...
    if( ( partitions[1].partition_type == 0x0b ) || ( partitions[0].partition_type == 0x0c ) ) {
        ...
        unsigned int starting_cluster = filebrowser();
        if( starting_cluster ) {
            printw("LOAD CLUSTERS: ");
            while( starting_cluster < 0xfff8 ) {
                printw("[%d] ", starting_cluster);
                starting_cluster = FAT32table[ starting_cluster ];
            }
        } else {
