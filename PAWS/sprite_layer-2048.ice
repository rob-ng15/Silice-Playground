...
algorithm sprite_layer(
    ...
    $$for i=0,12 do
        // Sprite Tiles
        simple_dualport_bram uint2048 tiles_$i$ <input!> [1] = uninitialised;
...
algorithm sprite_generator(
    ...
    uint4 xinsprite := ( 15  - ( ( __signed({1b0, pix_x}) - sprite_x ) >>> sprite_double ) );
    ...
    pix_visible := sprite_active && xinrange && yinrange && ( tiles.rdata0[ { sprite_tile_number, yinsprite, xinsprite }, 1 ] );
    ...
    while(1) {
        ...
        if( sprite_writer_active ) {
            switch( sprite_writer_sprite ) {
                $$for i=0,12 do
                    case $i$: {
                        tiles_$i$.wdata1[ { spriteline, 4b0000 } ,16 ] = sprite_writer_bitmap;
                    }
                $$end
            }
        }
