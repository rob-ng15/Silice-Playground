...
// BLIT - OUTPUT PIXELS TO BLIT A 16 x 16 TILE ( PARAM1 == 0 as 16 x 16, == 1 as 32 x 32, == 2 as 64 x 64, == 3 as 128 x 128 )
algorithm blit (
    input   int11   x,
    input   int11   y,
    input   int11   param0,
            ...
            while( gpu_active_y < gpu_max_y ) {
                while( gpu_active_x < gpu_max_x ) {
                    while( gpu_y2 < ( 1 << gpu_param1 ) ) {
                        bitmap_write = blit1tilemap.rdata0[15 - ( gpu_active_x >> gpu_param1 ),1];
                        gpu_y2 = gpu_y2 + 1;
                    }
                    gpu_active_x = gpu_active_x + 1;
                    gpu_y2 = 0;
