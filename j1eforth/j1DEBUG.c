...
static char opcode[16] = { 'T', 'N', '+', '&', '|', '^', '~', '=', '<', '}', '-', 'R', 'm', '{', 'D', '[' };
...
static void execute(int entrypoint)
{
  ...
  do {
    _pc = pc + 1;
    printf("\n pc = %04x insn = %04x ", pc, insn);
    if (insn & 0x8000) { // literal
      push(insn & 0x7fff);
      printf("L %04x ", insn & 0x7fff);
    } else {
      ...
      case 2: // call
        ...
        break;
      case 3: // alu
        printf("A op=%1c r2p=%1x t2s=%1x t2r=%1x s[t]=%1x ", opcode[(insn >> 8) & 0xf], (insn & 0x1000) == 0x1000, (insn & 0x80) == 0x80, (insn & 0x40) == 0x40, (insn & 0x20) == 0x20);
		if (insn & 0x1000) {/* r->pc */
			_pc = r[rsp] >> 1;
		}
        s = d[dsp];
