algorithm main(
    ...
    // interface
    sdram_r128w16_io sio_fullrate;
    sdram_r128w16_io sio_halfrate;
    sdram_half_speed_access sdaccess <@sdram_clock,!sdram_reset> (
            sd      <:> sio_fullrate,
            sdh     <:> sio_halfrate,
    );
    // algorithm
    sdram_controller_autoprecharge_r128_w16 sdram32MB <@sdram_clock,!sdram_reset> (
        sd        <:> sio_fullrate,
        sdram_cle :>  sdram_cle,
        sdram_dqm :>  sdram_dqm,
        sdram_cs  :>  sdram_cs,
...
circuitry SDRAMread( inout sd, inout cachebram, input addr ) {
    ...
    while( count < 8 ) {
        ...
        cachebram.wdata1 = { 1b1, physicaladdress[15,15], sd.data_out[{count,4b0000},16] };
